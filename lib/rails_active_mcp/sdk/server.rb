  ...
  module Sdk
    class Server
      ...
      def run_stdio
        ...
        warn "[#{Time.current}] [RAILS-MCP] FATAL: SDK Server crashed: #{e.message}"
        warn "[#{Time.current}] [RAILS-MCP] FATAL: #{e.backtrace.join("\n")}"
        raise
      ensure
      ...
      def setup_output_redirection
        ...
        warn "[#{Time.current}] [RAILS-MCP] INFO: Output redirection enabled. stderr -> #{stderr_log}"
      end
      ...
      def ensure_output_redirection_for_stdio
        ...
        if @stdout_buffer && !@stdout_buffer.string.empty?
          ...
          warn "[#{Time.current}] [RAILS-MCP] WARNING: Captured stdout during initialization: #{captured.inspect}"
        end

        # Restore original stdout for MCP communication, keep stderr redirected
        $stdout = @original_stdout
        warn "[#{Time.current}] [RAILS-MCP] INFO: stdout restored for MCP communication, stderr remains redirected"
      end
      ...
      def handle_rails_exception(exception, context)
        ...
        warn "[#{Time.current}] [RAILS-MCP] ERROR: MCP Exception: #{exception.message}"
        warn "[#{Time.current}] [RAILS-MCP] ERROR: #{exception.backtrace.join("\n")}" if ENV['RAILS_MCP_DEBUG'] == '1'

        # Log context for debugging
        return unless context && ENV['RAILS_MCP_DEBUG'] == '1'

        warn "[#{Time.current}] [RAILS-MCP] DEBUG: MCP Context: #{context.inspect}"
      end
      ...
      def log_mcp_calls(data)
        ...
        warn "[#{Time.current}] [RAILS-MCP] DEBUG: #{log_message}"

        # Log errors separately
        return unless data[:error]

        warn "[#{Time.current}] [RAILS-MCP] ERROR: MCP Call Error: #{data[:error]}"
      end
